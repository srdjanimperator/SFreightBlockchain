...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "sfd",
		Short:             "SmartFreight App Daemon (server)",
	...
	executor := cli.PrepareBaseCmd(rootCmd, "SF", app.DefaultNodeHome)
...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer) abci.Application {
	return app.NewSmartFreightApp(logger, db)
}
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		nsApp := app.NewSmartFreightApp(logger, db)
